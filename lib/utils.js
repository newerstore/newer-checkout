...
export function classifyProduct(rawTitle) {
  ...
  if (/training|treino/.test(t)) model = 'Treino';
  else if (/pre-match|pre match|pre jogo|pré-jogo|pre-jogo/.test(t)) model = 'Pré-jogo';
  else if (/\bhome\b/.test(t)) model = 'Home';
  ...
  return { model, isPlayer, isRetro, isFemale, price };
}
...
export function buildShopifyTitle(rawTitle, fallbackTeam) {
  ...
  const { model, isPlayer, isFemale } = classifyProduct(rawTitle);
