...
function detectExtraName(sourceTitle = '') {
  const t = normalizeText(sourceTitle);

  if (/polo/.test(t)) return 'Polo';
  if (/vest|colete/.test(t)) return 'Colete';
  if (/hooded|hoodie|sweater|moletom/.test(t)) return 'Moletom';
  if (/windbreaker|jacket|jaqueta/.test(t)) return 'Jaqueta';
  if (/baseball/.test(t)) return 'Baseball';
  if (/red black striped|red-black striped|striped|listrada|listrado/.test(t)) return 'Listrada';
  if (/\bred\b|vermelha|vermelho/.test(t)) return 'Vermelha';
  if (/\bblack\b|preta|preto/.test(t)) return 'Preta';
  if (/\bblue\b|azul/.test(t)) return 'Azul';
  if (/\bbrown\b|marrom/.test(t)) return 'Marrom';
  if (/limited edition|limited|limitada|limitado/.test(t)) return 'Limitada';
  if (/christ|redeemer|cristo/.test(t)) return 'Cristo Redentor';
  if (/all sponsors|sponsors|patrocinio|patrocinios/.test(t)) return 'Patrocinadores';
...
function classifyModelForTitle(sourceTitle = '') {
  ...
  const extraName = detectExtraName(sourceTitle);
...
function getTagsFromProduct(sourceProduct, model, extraName, isPlayer, isRetro, isFemale, isLongSleeve) {
  return uniqueTags([
    'brasileirao',
    normalizeTag(sourceProduct.fallbackTeam),
    normalizeTag(model),
    extraName ? normalizeTag(extraName) : null,
...
export function buildProductPayload(sourceProduct) {
  ...
  const extraName = localClassification.extraName;
