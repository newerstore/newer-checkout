...
async function fetchHtml(url) {
  const res = await fetch(url, {
    headers: {
      ...
      'pragma': 'no-cache',
      'referer': 'https://bluesky-footballshirt.com/'
    }
  });
...
function getMainProductImages($, url, title) {
  ...
  const productImages = cleanImages.filter(src =>
    src.includes('/products/') || src.includes('/photo/')
  );
...
export function isLikelyJersey(title = '') {
  ...
  // bloquear infantil
  if (
    /kids|kid|children|child|youth|junior|baby|infantil|juvenil/.test(t) ||
    /size\s*(16|18|20|22|24|26|28)/.test(t)
  ) {
    return false;
  }
